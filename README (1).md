...
- **Multi-tool agent**: 6 specialized tools — RAG retrieval, financial calculator, document comparator, web search fallback, Yahoo Finance live data, portfolio risk analyzer — all exposed via MCP server
...
- **Redis conversation store**: Persistent multi-turn storage with TTL, auto-fallback to in-memory, health checks
- **RAGAS evaluation**: Faithfulness, answer relevancy, context precision, context recall — automated + LLM-as-judge fallback
...
# 3. One-command startup (all 4 services: API + UI + MCP + Redis)
make up     # or: docker compose up --build

# 4. Or run locally (with Redis)
...
# 5. Run evaluations
make eval        # Legacy 18-metric evaluation
make eval-ragas  # RAGAS evaluation (faithfulness, relevancy, precision, recall)
```
...
|--------|----------|-------------|
| POST | `/api/v1/chat` | Main agent chat endpoint (Redis-backed conversations) |
| GET | `/api/v1/health` | Health check + Redis + RAGAS availability |
| GET | `/api/v1/trace/{conversation_id}` | Full conversation trace (from Redis) |
| GET | `/api/v1/conversations` | List recent conversation IDs |
| DELETE | `/api/v1/conversation/{conversation_id}` | Delete a conversation |
| GET | `/api/v1/metrics` | Store metrics and feature flags |
| POST | `/api/v1/evaluate` | Run legacy golden trace evaluation |
| POST | `/api/v1/evaluate/ragas` | Run RAGAS evaluation on all traces |
| POST | `/api/v1/evaluate/ragas/single` | Run RAGAS on a single query-answer-context |
...
| **Redis conversation store** | In-memory only, PostgreSQL | Redis provides persistence + TTL for production, with seamless in-memory fallback for development. |
...
**LangChain** · **LangGraph** · **Gemini 3.1 Flash Lite** · **FAISS** · **BM25** · **FastAPI** · **FastMCP** · **Streamlit** · **Docker** · **Redis** · **LangSmith** · **yfinance** · **RAGAS** · **NumPy** · **Pandas**
...
│   ├── redis_store.py           # Redis conversation store with in-memory fallback
...
├── api/
│   ├── main.py                  # FastAPI with Redis chat + RAGAS eval endpoints
│   ├── models.py                # Pydantic schemas
│   └── middleware.py            # Request logging + error handling
├── ui/
│   └── app.py                   # Streamlit chat + trace viewer
├── mcp_server/
│   ├── server.py                # FastMCP with all 5 tools (RAG, calc, compare, yahoo, portfolio)
│   ├── run.py                   # Entry point
│   └── __init__.py
...
│   └── ...
├── eval/                        # NEW: RAGAS evaluation
│   ├── __init__.py
│   └── ragas_eval.py            # RAGAS evaluator (faithfulness, relevancy, precision, recall)
├── evaluation/                  # Legacy evaluation (18 metrics)
...
├── docker-compose.yml           # API + UI + MCP + Redis
├── Dockerfile
├── Dockerfile.mcp
├── Makefile                     # Includes redis, eval-ragas, up/down targets
├── requirements.txt             # Includes redis>=5.0.0, ragas>=0.1.0
└── .env.example
```

## Redis Conversation Store

Multi-turn conversations are persisted in **Redis** with automatic fallback to in-memory storage.

### Features

| Feature | Description |
|---------|-------------|
| **Persistent storage** | Conversations survive API restarts |
| **TTL (72h default)** | Auto-expiry prevents storage bloat |
| **Sliding window** | Last 10 turns kept per conversation |
| **Health checks** | Latency monitoring with automatic fallback |
| **Thread-safe** | No race conditions in concurrent access |
...
REDIS_PORT=6379
REDIS_DB=0
CONVERSATION_TTL_SECONDS=259200  # 72 hours
```
...
```python
# Redis available → persist to Redis
# Redis down    → auto-fallback to in-memory (no crash)
# Redis recovers→ reconnect on next health check
```

**Files:**
- `agent/redis_store.py` — Core store with get/save/append/delete operations
- `docker-compose.yml` — Redis 7-alpine service with memory limits
- `api/main.py` — Integrated chat endpoint with Redis-backed sessions

## RAGAS Evaluation

Automated evaluation using the **RAGAS** framework with LLM-as-judge fallback.

### Metrics

| Metric | Description | Target |
|--------|-------------|--------|
| **Faithfulness** | Are claims grounded in retrieved contexts? | >= 0.6 |
| **Answer Relevancy** | Is the answer relevant to the question? | >= 0.6 |
| **Context Precision** | Are retrieved contexts relevant? | >= 0.6 |
| **Context Recall** | Does context contain needed information? | >= 0.6 |

### CLI Usage

```bash
# Run RAGAS on all golden traces
make eval-ragas

# Run on a single trace
make eval-ragas-single ID=ST-01

# Direct Python
python -m eval.ragas_eval --all
python -m eval.ragas_eval --trace-id ST-01
```
...
# Evaluate single query-answer-context
POST /api/v1/evaluate/ragas/single
Content-Type: application/json

{
  "query": "What was the repo rate in FY2023?",
  "answer": "The repo rate was 6.5%.",
  "contexts": ["RBI maintained the repo rate at 6.5% in FY2023"]
}
```

### Fallback Mode

If RAGAS is not installed, the evaluator automatically falls back to LLM-as-judge:

```python
# With RAGAS: full 4-metric evaluation
# Without RAGAS: faithfulness + heuristic relevancy/precision/recall
```

**Files:**
- `eval/ragas_eval.py` — RAGAS evaluator with fallback
- `eval/__init__.py` — Package init
- `api/main.py` — `/evaluate/ragas` and `/evaluate/ragas/single` endpoints

## MCP Server

All 5 tools are exposed as an MCP server for universal agent compatibility:

```python
# Any MCP client can call:
await search_financial_documents("RBI repo rate", top_k=5)      # RAG hybrid search
await calculate_financial_metric("growth_rate(4.0, 6.5)")       # Safe calculator
await compare_documents(doc_a="...", doc_b="...", metric="repo") # Document comparison
await yahoo_finance(ticker="RELIANCE.NS", operation="quote")     # Live stock data
await portfolio_analyzer(tickers="RELIANCE.NS,INFY.NS")         # Portfolio analysis
await list_available_tools()                                      # Discover all tools
```
