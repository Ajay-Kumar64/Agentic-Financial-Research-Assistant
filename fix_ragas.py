...
class RagasEvaluator:
    ...
    def evaluate_single(self, query, answer, contexts, trace_id="unknown", ground_truth=None):
        ...
        if not contexts:
            result.error = "No contexts provided"; result.latency_ms = int((time.time()-t0)*1000); result.passed = False; return result
        if not answer:
            result.error = "Empty answer provided"; result.latency_ms = int((time.time()-t0)*1000); result.passed = False; return result
        try:
            is_calc = (any("Calculation" in ctx or "Calc:" in ctx for ctx in contexts) or
            ...
            result.error = str(e); print(f"[RAGAS ERROR] {trace_id}: {e}")
        result.latency_ms = int((time.time()-t0)*1000); result.passed = result.overall_score >= 0.6
        return result
    ...
    def _evaluate_with_ragas(self, result, ground_truth=None):
        ...
        if ground_truth: data["ground_truth"] = [ground_truth]
        dataset = Dataset.from_dict(data)
        metrics = []
        for m in [faithfulness, answer_relevancy, context_precision, context_recall]:
            try:
                if callable(m): metrics.append(m())
                else: metrics.append(m)
            except: metrics.append(m)
        eval_kwargs = {"dataset": dataset, "metrics": metrics, "raise_exceptions": True}
        if self.ragas_version >= "0.2.0":
            try:
                from ragas.llms import LangchainLLMWrapper
                from ragas.embeddings import LangchainEmbeddingsWrapper
                eval_kwargs["llm"] = LangchainLLMWrapper(self.judge_llm)
                eval_kwargs["embeddings"] = LangchainEmbeddingsWrapper(self.embedder)
                print("[RAGAS] Using wrapped LLM + embedder")
            except ImportError:
                eval_kwargs["llm"] = self.judge_llm
                eval_kwargs["embeddings"] = self.embedder
                print("[RAGAS] Using raw LangChain objects")
        try:
            print("[RAGAS] Running evaluate()...")
            ragas_result = evaluate(**eval_kwargs)
            scores = ragas_result.to_pandas()
            print(f"[RAGAS DEBUG] Columns: {list(scores.columns)}")
    ...
    def _evaluate_fallback(self, result):
        ...
        for ctx in result.contexts:
            if qw & set(ctx.lower().split()): rc += 1
        result.context_precision = round(rc / len(result.contexts), 3) if result.contexts else 0.0
        akt = [w for w in aw if len(w) > 3]; cwt = 0
        for ctx in result.contexts:
            cl = ctx.lower()
            if any(t in cl for t in akt): cwt += 1
    ...
    def _evaluate_fallback_calculator(self, result):
        ...
        ac = result.answer
        if "=" in ac: ac = ac.split("=")[-1].strip()
        an = set(re.findall(r'\\\\d+\\\\.?\\\\d*', ac))
        cn = set()
        for ctx in result.contexts: cn.update(re.findall(r'\\\\d+\\\\.?\\\\d*', ctx))
        print(f"[RAGAS DEBUG] query_numbers={qn}, answer_numbers={an}, context_numbers={cn}")
        if qn:
            m = qn & cn; result.context_precision = round(len(m) / len(qn), 3)
            print(f"[RAGAS DEBUG] precision={result.context_precision}")
        else: result.context_precision = 1.0
        if an:
            m = an & cn; result.context_recall = round(len(m) / len(an), 3)
            print(f"[RAGAS DEBUG] recall={result.context_recall}")
        else: result.context_recall = 1.0
        if an & qn: result.answer_relevancy = 1.0
        elif an & cn: result.answer_relevancy = 0.8
        else: result.answer_relevancy = 0.5
        tool_outputs = [{"text_summary": ctx[:500]} for ctx in result.contexts]
        faith_score, _ = judge_faithfulness(result.answer, tool_outputs)
        result.faithfulness = round(faith_score, 3)
        result.fallback_faithfulness = round(faith_score, 3)
    ...
    def evaluate_from_agent_output(self, trace_id, query, output_state, ground_truth=None):
        ...
        for p in output_state.get("retrieved_passages", []):
            if isinstance(p, dict): contexts.append(p.get("text", ""))
        for ctx in output_state.get("retrieved_contexts", []):
            if isinstance(ctx, str) and len(ctx) > 10: contexts.append(ctx)
        comp = output_state.get("comparison_results")
        if isinstance(comp, str) and len(comp) > 10: contexts.append(comp)
        # STEP 4: Extract calculator from FINAL RESPONSE first
        if answer:
            ...
            if m:
                ...
                contexts.append(ct)
                print(f"[RAGAS DEBUG] extracted from final_response: {ct}")
                ...
                if m:
                    ...
                    contexts.append(ct)
                    print(f"[RAGAS DEBUG] extracted failed calc: {ct}")
                    for num in set(re.findall(r'\\\\d+\\\\.?\\\\d*', expr)):
                        contexts.append(num)
        # STEP 5: State calculator_results (fallback)
        if not any("Calculation" in c for c in contexts):
            for calc in output_state.get("calculator_results", []):
                if isinstance(calc, dict):
                    ...
                    if expr:
                        ...
                        if res is not None: nums += re.findall(r'\\\\d+\\\\.?\\\\d*', str(res))
                        for num in set(nums): contexts.append(num)
                elif isinstance(calc, str): contexts.append(calc)
        # STEP 6: Tool outputs (last resort)
        if not contexts:
            for output in output_state.get("tool_outputs", []):
                ...
                if isinstance(res, dict):
                    ...
                    if ts and len(ts) > 10: contexts.append(ts)
                elif isinstance(res, str) and len(res) > 10: contexts.append(res)
                ...
                if ts and isinstance(ts, str) and len(ts) > 10: contexts.append(ts)
        contexts = [c for c in contexts if c and len(c) > 0]
        print(f"[RAGAS] Extracted {len(contexts)} contexts for {trace_id}")
        print(f"[RAGAS DEBUG] All contexts: {contexts[:10]}")
...
def evaluate_golden_traces(trace_ids=None, output_path=None):
    ...
    if trace_ids: traces = [t for t in traces if t.get("id") in trace_ids]
    results, ragas_scores = [], []
    print(f"[RAGAS] Evaluating {len(traces)} traces..."); print("=" * 60)
    for i, trace in enumerate(traces):
        ...
        try:
            ...
            if ragas_result.overall_score > 0: ragas_scores.append(ragas_result.overall_score)
            status = "PASS" if ragas_result.passed else "FAIL"
            print(f"  {status} | Score: {ragas_result.overall_score:.3f}")
            if ragas_result.error: print(f"  ERROR: {ragas_result.error}")
            if ragas_result.faithfulness is not None: print(f"    Faithfulness: {ragas_result.faithfulness}")
            if ragas_result.answer_relevancy is not None: print(f"    Relevancy: {ragas_result.answer_relevancy}")
            if ragas_result.context_precision is not None: print(f"    Context Precision: {ragas_result.context_precision}")
            if ragas_result.context_recall is not None: print(f"    Context Recall: {ragas_result.context_recall}")
        except Exception as e:
            print(f"  ERROR: {e}")
            if ragas_result is None:
                results.append({"trace_id": trace_id, "error": str(e), "overall_score": 0.0, "passed": False})
        if i < len(traces) - 1: time.sleep(2)
    summary = {
    ...
    print("\\\\n" + "=" * 60); print("RAGAS EVALUATION SUMMARY"); print("=" * 60)
    print(f"Total: {summary['total_evaluated']}"); print(f"Passed: {summary['passed']}")
    print(f"Failed: {summary['failed']}"); print(f"Avg Score: {summary['avg_overall_score']:.3f}")
    return summary
...
def main():
    ...
    if args.all:
        ...
        result = evaluator.evaluate_single(query=args.query, answer=args.answer, contexts=contexts, trace_id="cli", ground_truth=args.ground_truth)
...
# Write the file
target = os.path.join(os.path.dirname(os.path.abspath(__file__)), "eval", "ragas_eval.py")
...
src = inspect.getsource(eval.ragas_eval.RagasEvaluator.evaluate_from_agent_output)
print(f"Has 'STEP 4': {'STEP 4' in src}")
print(f"Has 'Calculator trace detected': {'Calculator trace detected' in src}")